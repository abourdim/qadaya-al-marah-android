#!/usr/bin/env bash
# ============================================================================
#  Qadaya al-Marah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/41-qadaya-al-marah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.qadayaalmarah}"
AVD_NAME="${AVD_NAME:-qadaya_al_marah_test}"
...
cmd_test() {
  ...
  local out="/tmp/qadaya-al-marah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Qadaya al-Marah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
